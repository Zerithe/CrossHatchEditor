...
#define SHADOW_PACKED_DEPTH 0  // Set to 0 to use native shadow samplers
...
float PCFShadow(vec4 shadowCoord, float bias, vec2 texelSize) {
    ...
    for (int x = -1; x <= 1; ++x) {
        for (int y = -1; y <= 1; ++y) {
            vec2 offset = vec2(x, y) * texelSize;
            float depth = clamp(projCoords.z - bias, 0.0, 1.0);
            shadow += shadow2D(s_shadowMap, vec3(projCoords.xy + offset, depth));
            samples++;
        }
    }
    return shadow / float(samples);
